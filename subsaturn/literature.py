...
def load_ss(verbose=False):
    ...
    # Orbital velocity, scaled to earth 
    cut['pl_vorb'] = 30 * cut.st_mass**0.5 * cut.pl_orbsmax**-0.5
    cut['pl_vesc'] = 11.2 * cut.pl_masse**0.5 * cut.pl_rade**-0.5
...
def load_nonss(verbose=False):
    ...
    cut = cut[pl_denssig > 2]
    cut = cut[~cut.pl_name.str.contains('Kepler-128')]
...
#        if row.pl_name=="K2-24 c": import pdb;pdb.set_trace()
        ...
        # Orbital velocity, scaled to earth 
        pl_vorb = 30 * (st_mass / 1.0 * u.Msun)**0.5 * a**-0.5
        pl_vesc = 11.2 * pl_masse**0.5 * pl_rade**-0.5
        ...
        set_quantiles(df2, i, pl_vorb, 'pl_vorb')
        set_quantiles(df2, i, pl_vesc, 'pl_vesc')
...
def df_to_rv_table(df, tablefn,):
    ...
    with open(tablefn, 'w') as f:
        f.write('% pl_name pl_radius pl_mass pl_density pl_orbeccen pl_teq pl_fenv st_metfe\n')
...
def row_to_string(row):
    row['pl_fenvper'] = row['pl_fenv'] * 100 
    row['pl_fenvpererr1'] = row['pl_fenverr1'] * 100 
    row['pl_fenvpererr2'] = row['pl_fenverr2'] * 100 
    ...
    s+=r" & {pl_teq:.0f}"
    s+=r" & {pl_fenvper:.1f}^{{ +{pl_fenvpererr1:.1f} }}_{{ {pl_fenvpererr2:.1f} }}"
